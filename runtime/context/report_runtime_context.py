...
def build_report_runtime_context(target_ip: str | None = None) -> dict[str, Any]:
    ...
    # FASE 30I-D: Runtime Sensor Fusion — observed + derived + normalized contract
    try:
        ...
        ctx["sensor_contract_version"] = _sensor_contract["sensor_contract_version"]
        ctx["gpu_operational_summaries"] = _sensor_contract["gpu_operational_summaries"]
        ctx["source_quality"] = _sensor_contract["source_quality"]
        if isinstance(ctx["sensor_snapshot"], dict):
            ctx["sensor_snapshot"]["sensor_contract_version"] = _sensor_contract["sensor_contract_version"]
            ctx["sensor_snapshot"]["gpu_operational_summaries"] = _sensor_contract["gpu_operational_summaries"]
            ctx["sensor_snapshot"]["source_quality"] = _sensor_contract["source_quality"]
            ctx["sensor_snapshot"]["gpu_summary"] = _sensor_contract["gpu_summary"]
        ...
        observed.append("gpu_operational_summaries")
        observed.append("source_quality")
    ...
    # FASE 30H + 30I-D: expose evidence_catalog for evidence guard
    _sensor_evidence_catalog = ctx.get("evidence_catalog", {}) if isinstance(ctx.get("evidence_catalog"), dict) else {}
    ctx["evidence_catalog"] = {
        ...
        "gpu_online": _sensor_evidence_catalog.get("gpu_online"),
        "prometheus_targets": _sensor_evidence_catalog.get("prometheus_targets", {}),
